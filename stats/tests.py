...
class TestStringMethods(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        test_file_path = os.path.join(os.path.dirname(__file__), 'test_data.swift')
    ...
    def test_block_location(self):
        ...
        self.assertEqual((0, 5), first_block.line_range)

    def test_contained_blocks(self):
        first_block = self.parsed_file.top_level_blocks[0]
        self.assertEqual([(1,4), (3,3)], [x.line_range for x in first_block.contained_blocks])

        second_block = self.parsed_file.top_level_blocks[1]
        self.assertEqual([], [x.line_range for x in second_block.contained_blocks])

        third_block = self.parsed_file.top_level_blocks[2]
        self.assertEqual([(10,13)], [x.line_range for x in third_block.contained_blocks])
